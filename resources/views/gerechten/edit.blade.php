                ...
                <form action="{{ route('gerechten.update', $gerecht) }}" method="POST" class="space-y-8">
                            ...
                            <div>
                                ...
                                <select name="categorie_id"
                                    ...
                                    @foreach($categorieen as $cat)
                                        <option value="{{ $cat->id }}"
                                            {{ old('categorie_id', $gerecht->categorie_id) == $cat->id ? 'selected' : '' }}>
                                            {{ $cat->naam }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="bg-gray-50 p-6 rounded-lg">
                        ...
                        <div id="ingredienten-container" class="space-y-4">
                            @foreach($gerecht->ingredienten as $i => $ing)
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-white border rounded-lg">
                                    <div>
                                        <label class="block text-sm font-medium mb-1">Product</label>
                                        <select name="ingredienten[{{ $i }}][product_id]"
                                                required
                                                class="w-full rounded-lg border-gray-300">
                                            <option value="">Kies product...</option>
                                            @foreach($producten as $p)
                                                <option value="{{ $p->id }}"
                                                    {{ old("ingredienten.$i.product_id", $ing->product_id) == $p->id ? 'selected' : '' }}>
                                                    {{ $p->naam }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div>
                                        ...
                                        <input type="text"
                                               name="ingredienten[{{ $i }}][hoeveelheid]"
                                               value="{{ old("ingredienten.$i.hoeveelheid", $ing->hoeveelheid) }}"
                                               required
                                               class="w-full rounded-lg border-gray-300">
                                    </div>
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="number"
                                       ...
                                       value="{{ old('bereidingstijd_minuten', $gerecht->bereidingstijd_minuten) }}"
                            ...
                            <div>
                                ...
                                <textarea name="bereidingswijze"
                                          rows="6"
                                          class="w-full rounded-lg border-gray-300">{{ old('bereidingswijze', $gerecht->bereidingswijze) }}</textarea>
                            </div>
                        </div>
                    </div>
...
{{-- PRODUCT OPTIES VOOR NIEUWE RIJEN --}}
@php
    $options = '';
    foreach ($producten as $p) {
...
<script>
    ...
    document.querySelectorAll('.remove').forEach(btn =>
        btn.onclick = e => e.target.closest('.grid').remove()
    );
</script>
@endsection
